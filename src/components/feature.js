...
const Feature = ({ feature, projects }) => {
    ...
    <Fragment>
        <aside className="feature">
            <figure>
                {activeProject?.display?.image && <img src={"/projects/"+activeProject.display.image} alt="" />}
            </figure>
            <section>
                <nav className="links">
                    {
                        projects[feature].points && Object.keys(projects[feature].points).map(title => 
                            <button key={title} className={title === activeTab ? 'active' : ''} onClick={() => setActiveTab(title)}>
                                {title}
                            </button>
                        )
                    }
                ...
                {activeProject?.text?.map(paragraph => <p key={paragraph }>{paragraph}</p>)}
            </section>
        </aside>
        <style jsx>{`
            .feature {
